...
# return number of tokens in the prompt list
...
def generate_response(input):
    ...
    if stage == 1:
        messages = systemContext + firstContext
    elif stage == 2:
        messages = systemContext + secondContext
    elif stage == 3:
        messages = systemContext + thirdContext
    elif stage == 4:
        messages = systemContext + fourthContext
    elif stage == 5:
        messages = systemContext + fifthContext
    else:
        messages = systemContext + donePrmpt
    ...
    # count tokens used by current input
    total_tokens = token_count(messages)
    # keep reducing input as long as it is equal to or over the limit
    while (total_tokens >= MAX_TOKENS):
        messages = token_reduction(messages)
        total_tokens = token_count(messages)
